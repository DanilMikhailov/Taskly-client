...
import { Box, Heading, Center, Input, Stack, FormControl, Button, Link, Flex, Text, FormErrorMessage } from '@chakra-ui/react';
...
export default function Profile() {
  ...
  const [files, setFiles] = useState([]); // Измените на массив
  ...
  const doSubmit = async values => {
    try {
      if (files.length > 0) {
        ...
        if (newUrl) {
          values.avatar = newUrl; // Устанавливаем URL изображения
        }
      }
  ...
  const handleFileUpload = async files => {
    ...
    try {
      ...
      console.log(error);
      throw error; // Используйте throw для выброса ошибки
    }
  };
  ...
  return (
    <Box p='3' maxW='lg' mx='auto'>
      <DeleteConfirmation alertTitle='Delete Account' handleClick={handleDeleteUser} isOpen={isOpen} onClose={onClose} />
      ...
      <Stack gap='4' mt='5'>
        <Link as={RouterLink} to='/create-task' p='2' bg='green.500' rounded='lg' textTransform='uppercase' textAlign='center' textColor='white' fontWeight='semibold' _hover={{ bg: 'green.600' }}>
          Create New Task
        </Link>
        <Flex justify='space-between'>
          <Text as='span' color='red.600' cursor='pointer' onClick={onOpen}>Delete Account</Text>
          <Text as='span' color='red.600' cursor='pointer' onClick={handleSignOut}>Sign Out</Text>
        </Flex>
        <Text textAlign='center'>
          <Link as={RouterLink} to='/tasks' color='teal' _hover={{ textDecor: 'none' }}>Show Tasks</Link>
        </Text>
      </Stack>
    </Box>
  );
